...
pub struct ItemsItemsLeft {
    pub items: Vec<Item>,
    pub items_left: i64
}

/// Returns items and items left
...
///
pub fn all(options: Option<FindOptions>, connection: &Conn) -> Result<ItemsItemsLeft, Error> {
    ...
    match items {
        Ok(obj) => {
                ...
                Ok(ItemsItemsLeft{items: obj.clone(), items_left: items_left})
            },
        Err(err) => Err(err)
    }
}

pub fn get(id: ObjectId, connection: &Conn) -> Result<Option<Item>, Error> {
    match connection
...
pub fn insert(items: Item, connection: &Conn) -> Result<Item, Error> {
    let insertable = InsertableItem::from_item(items.clone());
                    ...
                    .insert_one(model_doc, None)
                {
                    Ok(res) => match res.inserted_id {
                        Some(res) => match bson::from_bson(res) {
                            Ok(res) => Ok(InsertableItem::assign_id(insertable, res)),
...
pub fn update(id: ObjectId, items: Item, connection: &Conn) -> Result<Item, Error> {
    let mut new_item = items.clone();
    new_item.id = Some(id.clone());
    match bson::to_bson(&new_item) {
        Ok(model_bson) => match model_bson {
            bson::Bson::Document(model_doc) => {
