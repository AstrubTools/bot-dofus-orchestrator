...
#[get("/")]
pub fn all(connection: Conn) -> Result<Json<Vec<Session>>, Status> {
    match sessions::repository::all(None, &connection) {
        Ok(res) => Ok(Json(res.sessions)),
...
#[get("/<skip>/<batch>")]
pub fn page(skip: Option<String>, batch: Option<String>, connection: Conn) -> Result<Json<sessions::repository::SessionsItemsLeft>, Status> {
    let mut options = FindOptions::new();
    options.skip = Some(skip.unwrap().parse::<i64>().expect("Na"));
    options.limit = Some(batch.unwrap().parse::<i64>().expect("Na"));
    match sessions::repository::all(Some(options), &connection) {
        Ok(res) => Ok(Json(res)),
        Err(err) => Err(error_status(err)),
    }
}
...
#[get("/<id>")]
pub fn get(id: String, connection: Conn) -> Result<Json<Session>, Status> {
    match ObjectId::with_string(&String::from(&id)) {
        Ok(res) => match sessions::repository::get(res, &connection) {
...
#[post("/", format = "application/json", data = "<sessions>")]
pub fn post(
    ...
    match sessions::repository::insert(sessions.into_inner(), &connection) {
        Ok(res) => {
...
#[put("/<id>", format = "application/json", data = "<sessions>")]
pub fn put(
    id: String,
    sessions: Json<Session>,
    connection: Conn,
    ...
    match ObjectId::with_string(&String::from(&id)) {
        Ok(res) => match sessions::repository::update(res, sessions.into_inner(), &connection) {
            Ok(res) => {
                if !server.inner().lock().unwrap().out.is_none() {
                    ...
                    // Get the Json<Session>> inside Created with .0 and converts it to string to send
...
pub fn delete(
    ...
    match ObjectId::with_string(&String::from(&id)) {
        Ok(res) => match sessions::repository::delete(res, &connection) {
            Ok(_) => {
                if !server.inner().lock().unwrap().out.is_none() {
...
pub fn delete_all(
    ...
    match sessions::repository::delete_all(&connection) {
        Ok(_) => {
            if !server.inner().lock().unwrap().out.is_none() {
...
pub fn count(connection: Conn) -> Result<Json<i64>, Status> {
    match sessions::repository::count(&connection) {
        Ok(res) => Ok(Json(res)),
        Err(err) => Err(error_status(err)),
    }
}