...
#[get("/")]
pub fn all(connection: Conn) -> Result<Json<Vec<Task>>, Status> {
    match tasks::repository::all(None, &connection) {
        Ok(res) => Ok(Json(res.tasks)),
...
#[get("/<skip>/<batch>")]
pub fn page(skip: Option<String>, batch: Option<String>, connection: Conn) -> Result<Json<tasks::repository::TasksItemsLeft>, Status> {
    let mut options = FindOptions::new();
    options.skip = Some(skip.unwrap().parse::<i64>().expect("Na"));
    options.limit = Some(batch.unwrap().parse::<i64>().expect("Na"));
    match tasks::repository::all(Some(options), &connection) {
        Ok(res) => Ok(Json(res)),
        Err(err) => Err(error_status(err)),
    }
}
...
#[get("/<id>")]
pub fn get(id: String, connection: Conn) -> Result<Json<Task>, Status> {
    match ObjectId::with_string(&String::from(&id)) {
        Ok(res) => match tasks::repository::get(res, &connection) {
...
#[post("/", format = "application/json", data = "<tasks>")]
pub fn post(
    tasks: Json<Task>,
    connection: Conn,
    ...
    match tasks::repository::insert(tasks.into_inner(), &connection) {
        Ok(res) => {
...
#[put("/<id>", format = "application/json", data = "<tasks>")]
pub fn put(
    id: String,
    tasks: Json<Task>,
    connection: Conn,
    ...
    match ObjectId::with_string(&String::from(&id)) {
        Ok(res) => match tasks::repository::update(res, tasks.into_inner(), &connection) {
            Ok(res) => {
                if !server.inner().lock().unwrap().out.is_none() {
                    ...
                    // Get the Json<Task>> inside Created with .0 and converts it to string to send
...
pub fn delete(
    ...
    match ObjectId::with_string(&String::from(&id)) {
        Ok(res) => match tasks::repository::delete(res, &connection) {
            Ok(_) => {
                if !server.inner().lock().unwrap().out.is_none() {
...
pub fn delete_all(
    ...
    match tasks::repository::delete_all(&connection) {
        Ok(_) => {
            if !server.inner().lock().unwrap().out.is_none() {
...
pub fn count(connection: Conn) -> Result<Json<i64>, Status> {
    match tasks::repository::count(&connection) {
        Ok(res) => Ok(Json(res)),
        Err(err) => Err(error_status(err)),
    }
}